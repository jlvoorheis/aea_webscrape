...
all_aea %>% 
  ...
  group_by(day_in_year) %>% summarise(diff_ads = mean(total_ads - lag(total_ads), na.rm=T), date = date[2])%>%
  ...
  labs(title="Difference in Cumulative Number of JOE Ads, 2019 vs. 2020 to Date",
       x="Date", y="Cumulative YOY Change in Jobs on AEA")
...
all_aea %>% 
  ...
  arrange(day_in_year, year) %>%
  group_by(day_in_year) %>% summarise(diff_ads = mean(total_ads - lag(total_ads), na.rm=T), date = date[2],
                                      diff_ads_pct = mean((total_ads - lag(total_ads))/lag(total_ads), na.rm=T))%>%
  filter(!is.na(diff_ads_pct)) %>% 
  ggplot() + 
  geom_line(aes(x=date, y=100*diff_ads_pct))+theme_bw()+
  labs(title="Percent Change in Cumulative Number of JOE Ads, 2019 vs. 2020 to Date",
       x="Date", y="Cumulative YOY Change in Jobs on AEA")
